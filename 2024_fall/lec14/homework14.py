import datetime, gtts, bs4, random, speech_recognition, chardet
...
def tell_me_a_joke(lang, audiofile):
    ...
    @params:
    lang (str) - language ('en', 'ja', 'zh')
    audiofile (str) - output audio filename
    '''
    ...
    try:
        ...
        print(f"Error: {e}. Ensure detect_encoding is defined.")
        return
    ...
    if not jokes:
        print(f"Error: No jokes found in {filename}")
        return

    joke = random.choice(jokes).strip()
    print(f"Joke ({lang}):", joke)

    try:
        tts = gtts.gTTS(joke, lang=lang)
        tts.save(audiofile)
        print(f"Audio file created: {audiofile}")
